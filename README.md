...
-- Canto superior direito — imagem como handle de arrastar
...
local function removeHighlight(object)
	if not object then return end
	local h = object:FindFirstChild("ESPHighlight")
	if h then h:Destroy() end
end

local function createNameLabel(object)
	if not object then return end
	if object:FindFirstChild("ESPNameLabel") then return end

	local anchorPart = object.PrimaryPart or object:FindFirstChildWhichIsA("BasePart")
...
local function removeNameLabel(object)
	if not object then return end
	local b = object:FindFirstChild("ESPNameLabel")
	if b then b:Destroy() end
end

local function clearESP()
	for obj in pairs(trackedTraps) do
		pcall(removeHighlight, obj)
		pcall(removeNameLabel, obj)
	end
	trackedTraps = {}
end

local function disconnectESPConnections()
	if ignoreConnection then ignoreConnection:Disconnect(); ignoreConnection = nil end
	if workspaceChildConnection then workspaceChildConnection:Disconnect(); workspaceChildConnection = nil end
	if mapAddedConnection then mapAddedConnection:Disconnect(); mapAddedConnection = nil end
	if mapRemovedConnection then mapRemovedConnection:Disconnect(); mapRemovedConnection = nil end
end

local function setupTrapESP(obj)
	if not espEnabled then return end
	if not obj then return end
	if obj.Name ~= "Trap" then return end
	if not obj:IsA("Model") then return end
	if trackedTraps[obj] then return end

	trackedTraps[obj] = true
	task.wait(0.05)

	if not espEnabled then
		trackedTraps[obj] = nil
		return
	end
...
local function connectIgnoreFolder(ignoreFolder)
	if not espEnabled then return end
	...
	if ignoreConnection then
		ignoreConnection:Disconnect()
	end

	ignoreConnection = ignoreFolder.ChildAdded:Connect(function(obj)
		if not espEnabled then return end
		if obj.Name == "Trap" and obj:IsA("Model") then
			task.wait(0.05)
			setupTrapESP(obj)
...
local function monitorTraps()
	task.spawn(function()
		...
		while espEnabled and waited < 30 do
			local ignore = Workspace:FindFirstChild("IGNORE")
			if ignore then
				connectIgnoreFolder(ignore)
				return
			end
			task.wait(0.5)
			waited = waited + 0.5
		...
		if workspaceChildConnection then
			workspaceChildConnection:Disconnect()
		end

		workspaceChildConnection = Workspace.ChildAdded:Connect(function(child)
			if not espEnabled then return end
			if child.Name == "IGNORE" then
				connectIgnoreFolder(child)
			end
		end)
	end)
end
...
local function watchForRound()
	...
	local mapsFolder = Workspace:WaitForChild("MAPS", 30)
	if not espEnabled then return end

	if not mapsFolder then
		task.delay(5, watchForRound)
		return
	end

	if mapsFolder:FindFirstChild("GAME MAP") then
		task.wait(2)
		if espEnabled then
			monitorTraps()
		end
	...
	if mapAddedConnection then mapAddedConnection:Disconnect() end
	if mapRemovedConnection then mapRemovedConnection:Disconnect() end

	mapAddedConnection = mapsFolder.ChildAdded:Connect(function(child)
		if not espEnabled then return end
	...
	mapRemovedConnection = mapsFolder.ChildRemoved:Connect(function(child)
		if not espEnabled then return end
		if child.Name == "GAME MAP" then
			resetESP()
		end
